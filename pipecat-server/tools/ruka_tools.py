"""ChatRuka tools — HTTP bridge to Next.js /api/voice-tools + RTVI UI push."""
...
class RukaTools:
    ...
    async def add_to_cart(self, params: FunctionCallParams) -> None:
        ...
        await params.result_callback(
            {"added": True, "name": card.get("name"), "quantity": quantity, "shown_on_screen": True}
        )

    async def open_checkout(self, params: FunctionCallParams) -> None:
        await self._push({"type": "open_checkout"})
        await params.result_callback(
            {"opened": True, "note": "Checkout drawer opened on screen."}
        )
    ...
    async def show_checkout_form(self, params: FunctionCallParams) -> None:
        ...
        await params.result_callback(
            {"shown": True, "step": step, "note": "In-chat checkout form is on screen."}
        )
    ...
    async def suggest_gift_message(self, params: FunctionCallParams) -> None:
        ...
        await params.result_callback(
            {"applied": True, "message": message, "shown_on_screen": True}
        )
    ...
    async def find_delivery_cities(self, params: FunctionCallParams) -> None:
        ...
        await params.result_callback(
            {"matches": [c.get("name") for c in cities[:8]], "serviceable": len(cities) > 0}
        )
    ...
    def schemas(self) -> list[FunctionSchema]:
        return [
            FunctionSchema(
                name="search_gifts",
                description="Find gifts; cards appear on the caller's screen.",
                properties={
                    "query": {"type": "string"},
                    "occasion": {
                        "type": "string",
            ...
            FunctionSchema(
                ...
                description="Look up a Sri Lankan delivery city.",
            ...
            FunctionSchema(
                name="check_delivery",
                description="Check delivery availability and fee for city + date.",
                properties={
                    "city": {"type": "string"},
            ...
            FunctionSchema(
                name="track_order",
                description="Track a Kapruka order by number.",
    ...
    def register_on_llm(self, llm: Any) -> None:
        """Register function handlers on Gemini Live or OpenAI LLM service."""
